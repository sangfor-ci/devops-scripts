...
def get_options():
    ...
    parser.add_argument('-l', '--webdav-url', type=str, help="Nexus 推送URL",
                        default="http://127.0.0.1:8080/webdav/")
    parser.add_argument('-g', '--group', type=str, help="需要上传的制品包的组,也是远程仓库的根路径", default="common")
    parser.add_argument('-u', '--auth', type=str, help="Nexus认证信息,例如admin:password", default="superman:string123.")
...
class WebdavUploader(object):
    ...
    def __init__(self, opts):
        ...
        """
        self.options = opts
        self.webdav_url = opts.webdav_url
        self.group = opts.group
        self.dir = opts.dir
        self.file = opts.file
        self.curl_bin = opts.curl_bin
        self.webdav_auth = opts.auth
    ...
    def _upload_file(self, filepath=None, artifact="temp"):
        ...
        cmd_shell = f"{self.curl_bin} --progress-bar -k "
        cmd_shell += f"'{self.webdav_url}/{self.group}/{artifact}' "
        cmd_shell += f"-u {self.webdav_auth} "
        cmd_shell += f"-T {filepath}"
...
if __name__ == '__main__':
    options = get_options()
    builder = WebdavUploader(opts=options)
