...
# TODO collect libs
 mkdir -p ${curdir}/${NGINX_ALIAS_NAME}/lib64 && cd ${curdir}/${NGINX_ALIAS_NAME}/lib64 && \
    cp /usr/local/modsecurity/lib/libmodsecurity.so.3.*  .  && \
    cp /usr/lib64/libyajl.so.2.* . && \
    cp /usr/lib64/libGeoIP.so.1.*  . && \
    cp /usr/local/lib/libfuzzy.so.2.* . && \
    cp /usr/lib64/liblmdb.so.0.* . && \
    cp /usr/lib64/libxslt.so.1.* . && \
    cp /usr/lib64/libexslt.so.0.* . && \
    cp /usr/lib64/liblmdb.so.0.* . && \
    cp ${curdir}/compile/luajit/lib/libluajit-5.1.so.2.1.0   .
